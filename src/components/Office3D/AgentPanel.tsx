  ...
  return (
    <div className="absolute right-0 top-0 h-full w-96 bg-black/90 backdrop-blur-md text-white p-6 shadow-2xl border-l border-white/10">
      ...
      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-6 ${getStatusBgColor()}`}>
        <div className={`w-2 h-2 rounded-full ${safeState.status === 'thinking' ? 'animate-pulse' : ''}`} style={{ backgroundColor: agent.color }}></div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white/5 p-3 rounded-lg">
            ...
            <p className="text-lg font-bold capitalize">{safeState.model || 'N/A'}</p>
          </div>

          {/* Tokens/hour */}
          <div className="bg-white/5 p-3 rounded-lg">
            <p className="text-xs text-gray-400 mb-1">Tokens/hour</p>
            <p className="text-lg font-bold">{safeState.tokensPerHour?.toLocaleString() || '0'}</p>
          </div>

          {/* Tasks in queue */}
          <div className="bg-white/5 p-3 rounded-lg">
            <p className="text-xs text-gray-400 mb-1">Queue</p>
            <p className="text-lg font-bold">{safeState.tasksInQueue || 0} tasks</p>
          </div>

          {/* Uptime */}
          <div className="bg-white/5 p-3 rounded-lg">
            <p className="text-xs text-gray-400 mb-1">Uptime</p>
            <p className="text-lg font-bold">{safeState.uptime || 0} days</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <div className="bg-white/5 p-3 rounded-lg text-sm">
            ...
            <p>Started: {safeState.currentTask || 'Processing data'}</p>
          </div>
          <div className="bg-white/5 p-3 rounded-lg text-sm">
            <p className="text-gray-400 text-xs mb-1">1 hour ago</p>
            <p>Switched model to {safeState.model}</p>
          </div>
        </div>
      </div>
